...
-- Index principal: búsqueda por torneo + estado
CREATE INDEX idx_preinscripciones_torneo_estado
ON preinscripciones_torneo(torneo_id, estado);
...
-- Index compuesto para admin dashboard (torneo + estado + fecha)
CREATE INDEX idx_preinscripciones_dashboard
ON preinscripciones_torneo(torneo_id, estado, created_at DESC);
...
-- Policy 2: Only admins and organizers can SELECT
-- Uses helper function to check user role
CREATE POLICY "Admins and organizers can view preinscripciones"
  ON preinscripciones_torneo
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      ...
      AND usuarios.rol IN ('admin', 'organizador')
    )
  );

-- Policy 3: Only admins can UPDATE (change estado, add notas)
CREATE POLICY "Only admins can update preinscripciones"
  ON preinscripciones_torneo
  FOR UPDATE
  TO authenticated
  USING (
